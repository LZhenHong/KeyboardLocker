...
- `UI/`:SwiftUI popover 层,承载于 AppKit `NSPopover` + `NSHostingController`(App 入口保持 accessory `@main`,不改成 SwiftUI `App`/`MenuBarExtra`,以免动到 activation policy、services provider 与 URL handling 三条已测路径)。`PopoverPresenter` 持有 `.transient` 的 `NSPopover`,`toggle`/`show`/`close` 锚定到 status item button。`PopoverRootView` 用轻量 `@State` enum 在 `StatusPage` / `SettingsPage` / `StatsPage` 间切换(非 `NavigationStack`;FAQ 内容超出 320pt popover 的合理承载,放在独立窗口)。`AppUIStore`(`ObservableObject`,部署到 macOS 13)只把 `AppCoordinator.Snapshot` 桥成可观察状态并转发动作,不含领域逻辑,由 controller 调 `receive(_:)` 推送而非自行抢占 observer;并提供纯呈现路由(`recoveryActions`、`canRunSafetyCheck`)。`StatusPage` 显示锁状态卡片、由权威 `autoUnlockTargetDate` 派生的倒计时、起点、active 解锁热键、unlocked 时的上次解锁记录(原因 + 时刻)、主动作与逐状态恢复按钮,底部工具条含手势提示(跟随当前可用动作:unlocked 且配置了锁定热键时提示锁定手势,locked 时提示解锁手势,设置不可读时不显示)、统计页入口(chart.bar)、FAQ 窗口入口(questionmark.circle)与 `⋯` overflow(ready 且 unlocked 时提供 5/10/30 分钟快捷定时锁,一次性覆写本轮 auto-unlock,不回写设置)。`StatsPage` 显示锁定统计:`onAppear` 时经 `AppCoordinator.loadLockHistory()` 拉取 Agent 有界历史(`HistoryState` idle/loading/loaded/unavailable,失败显示 unavailable 而非空表),由 `LockHistoryStats` 纯函数(注入 now/calendar,可单测)派生今日锁定次数/总时长、本周(周日–周六)每日堆叠柱状图(每日柱按解锁方式分段着色,色板与下方占比线一致,今日实心、往日降不透明度、未来日标签淡化,附周总数行)与按解锁方式的分段色线(4pt capsule,左右各缩 8pt,每段一个方法色,宽度 = 占全部解锁的比例)+ 单行铺开图例(色点 + 名称 + 次数,最左项贴左缘、最右项贴右缘,与色线同跨度)(条目归属其结束日、时长截断到非负、"周"即与图表同源的本周日历周,周日起,locale 无关),空历史显示引导文案。`SettingsPage` 带返回导航,编辑持久化设置:全局锁定热键开关与录制(默认建议 ⌃⌘K,关闭即注销)、解锁热键录制、解锁短语开关与文本(共享 `validated()` 护栏即时反馈)、auto-unlock 档位、`.disabled` 二次确认、Feedback 区的提示音与被拦输入提示开关;两支热键在录制 / 启用时都会经 `SystemShortcutConflicts` 做冲突检查——对照 `com.apple.symbolichotkeys` 里的已启用系统快捷键(Spotlight、输入法切换、截图、Mission Control 等,常见 ID 有社区考据名,未知 ID 走通用文案)以及与另一支热键重复(会变成 toggle),命中只显示非阻塞警告,不拒绝写入(plist 格式无官方文档,best-effort);读取失败显示 unavailable 而非默认值,locked 时提示"下次锁生效"。FAQ 展示为独立窗口而非 popover 页:`FAQWindowPresenter` 持有单个可复用 `NSWindow`(关闭仅隐藏,保留用户调整过的位置与尺寸,呈现前先 `popoverPresenter.close()` 避免遮挡),`FAQView` 渲染随包的用户 FAQ——真相源是仓库根目录的 `FAQ.md`(经经典 file reference 打进 App bundle 与测试 bundle,与 GitHub 页面不会漂移),`FAQContent` 只接受文档使用的受约束 Markdown 子集(`##` section、`**Q:**` 条目、段落、`-`/`1.` 列表、围栏代码块;preamble 成为窗口标题下的 intro,inline span 留给视图),加载失败显示 unavailable 而非空页。每条 Q&A 是一行 `DisclosureGroup` 折叠行(问题常驻、答案折叠,20+ 条扫读为扁平列表),答案文本可选中;`ProseHighlight` 把 inline 强调渲染成两档高亮——`**strong**` 为 accent(动作/关键事实)、`*emphasis*` 为橙色加粗(注意事项,斜体刻意换成加粗——经改写 presentation intent 而非指定具体字号,保持容器字级),解析失败退化为无样式正文;`FAQContentTests` 的 guard 测试直接对随包文档断言 intro、section 与条目数量,`ProseHighlightTests` 锁定两档渲染契约,文档与高亮语义改动必须同步过测试。`HotkeyCapture` 是纯翻译(捕获→候选热键,复用 `Hotkey.validated()`,丢弃 matcher 忽略的修饰键,Escape/Return 结束录制),`HotkeyRecorderField`/`HotkeyRecorderView` 是其 AppKit 捕获宿主;`PopoverActions` 是 AppKit 确认动作的注入缝。
