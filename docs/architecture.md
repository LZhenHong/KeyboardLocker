...
- **各进程独立读 `UserDefaults` → 设置漂移。** 旧方案每个进程各持一份配置,CLI 改了热键 App 不知道。
  → **教训**:设置也只能有一个真相源(Agent)。读写都经同一 XPC 边界:读取用 `currentSettings`,写入用 `applySettings`(由 Agent 校验并持久化);wrapper 永不拥有 store。
...
- 单次定时锁(`beginTimedLock`,protocol 1.9)把 safety check 的覆写模式一般化：沿用 Agent 持久化设置,仅把该轮锁的 auto-unlock 覆写为调用方给定的时长——由 `KeyboardLockerSettings.validated()` 护栏强制有限、取整并落入 `allowedAutoUnlockRange`(5–3600 秒),不回写 settings store;已锁时原子返回 alreadyLocked 且不修改既有锁,override 绝不作用于当前锁。`interactively` 为 true 时与 interactive lock 一致:只有真正完成 unlocked → locked 转换的那一轮才接受 `Ctrl+C`。这是 `lock` 幂等语义的延伸,不是一次 settings 写入。
...
- 键入短语解锁是 locked 期间引擎侧的第三种输入手势(与解锁热键、交互 `Ctrl+C` 并列):`KeyboardLockerSettings.unlockPhrase` 非 nil 时,`LockEngine` 用滚动后缀缓冲匹配本轮 active settings 的短语。缓冲是瞬态输入状态——不落盘、不进 snapshot、不写日志,随每代锁创建/销毁重建;修饰键组合与非字符键重置缓冲,退格编辑,autorepeat 忽略。短语值不经通知、Widget 或 CLI 展示(通知与 CLI 只提示手势存在)。它经既有 settings 读写链路(protocol 1.8)流转,不是新的锁操作;写入沿用既有语义——locked 时只落盘,seed 下一代锁。
- 输入手势的不对称是刻意的:解锁热键由 Agent 引擎在 locked 时匹配(锁内唯一可靠的出口),而**全局锁定热键由 App 进程用 Carbon `RegisterEventHotKey` 注册**(`KeyboardLockerSettings.lockHotkey`,nil 即关闭)。它不需要 Accessibility 权限,但只在 App 运行时有效;locked 期间该组合被 event tap 当作普通按键吞掉,出口仍是解锁手势。锁定热键只是 wrapper 的意图翻译(一次 `lock()` XPC 调用),不进入 Agent;不要因为对称性把它移进引擎——那会让 Agent 常驻一个被动 tap,扩大持权限进程的攻击面。注册失败(组合被占用)由 App 告知用户更换,不影响任何锁行为。
