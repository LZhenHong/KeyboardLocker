...
- `LockStatusNotifier.swift`:Agent 进程内的 "Keyboard Locked" 系统通知 owner。挂在 engine 的 `broadcastStateChange` 副作用缝上(live 实现里先广播再驱动 notifier),状态转换完成后读取进程内 `statusSnapshot` 拼装正文(解锁热键 + auto-unlock 截止时间),经 `UNUserNotificationCenter` 投递/移除固定 identifier 的通知;`Unlock Now` action 由 Agent 本地执行幂等 unlock。首次锁定时请求通知权限,拒绝则静默降级。`start()` 在 bootstrap 时安装 action category 并清除上一代 Agent 退出留下的残留通知。通知生命周期与锁生命周期由此绑定在同一进程,不会比锁活得更久。
